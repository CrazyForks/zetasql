    ...
    case FN_MAP_REPLACE_KV_PAIRS: {
      ZETASQL_RET_CHECK_GE(arguments.size(), 3)
          ...
          << " requires at least 3 arguments: a map, and at least one "
             "key/value pair.";
      ZETASQL_RET_CHECK_EQ((arguments.size() - 1) % 2, 0)
          << node.function()->Name()
...
absl::StatusOr<std::vector<FunctionArgumentType>> BuildExpandedArgumentTypes(
  ...
  for (const auto& arg_info : arguments) {
    ...
    options.set_argument_name(arg_info.name, zetasql::kPositionalOrNamed);
    if (arg_info.expr != nullptr) {
      ...
      const zetasql::ResolvedInlineLambda* lambda = arg_info.inline_lambda;

      zetasql::FunctionArgumentTypeList lambda_arg_types;
      for (const zetasql::ResolvedColumn& arg_col : lambda->argument_list()) {
      ...
      zetasql::FunctionArgumentType lambda_body_type(body_type,
                                                       /*num_occurrences=*/1);
...
class VariadicFunctionSignatureExpanderVisitor
 ...
 private:
  ...
  PostVisitResolvedFunctionCall(
    ...
    ZETASQL_ASSIGN_OR_RETURN(std::string generated_sql,
                     GenerateSqlForVariadicFunction(*node, arguments));

    ZETASQL_ASSIGN_OR_RETURN(std::vector<FunctionArgumentType> new_arg_types,
